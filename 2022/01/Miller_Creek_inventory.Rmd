...
We collected `r nspecimens` morphological specimens representing `r nphyl` phyla, `r nclas` classes, `r nord` orders, `r nfam` families, `r ngen` genera, and `r nspp` species (Table \@ref(tab:specimentable)). 

```{r specimentable}

knitr::kable(sr[,c("phylum", "scientific_name", "waterbody", "GUID")], 
  longtable = TRUE, 
  booktabs = TRUE,
  col.names = c("Phylum", "Identification", "Water body", "GUID"),
  row.names = FALSE,
  caption = "Specimen records. The GUIDs are hyperlinked to the complete specimen records on Arctos."
)
```

```{r chordplot, fig.cap="Chord plot of identifications obtained by metabarcoding from the Miller Creek watershed in 2021. Sizes of circles correspond to numbers of sequence reads observerd.", out.width="100%"}
...
The water mite *Huitfeldtia rectipes* and the chironomid midge *Psectrocladius simulans* have holarctic distributions [@Hayford2005; @Smithetal2011], but our observations in the Miller Creek watershed appear to be the first documentation of these species in Alaska.

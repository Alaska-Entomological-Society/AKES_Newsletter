...
As in @Massengill2014 and @Massengill2017, the metric used will be simple presence or absence of taxa within the lakes before and after treatment.

We formatted occurrence records to be published to GBIF using the guidance of @Anderssonetal2020.

## Results

Specimen data and images are available via an Arctos project (<https://arctos.database.museum/project/10003613>). Specimen records are available via a search for records from this project (<https://arctos.database.museum/SpecimenResults.cfm?project_id=10003613>). All Arctos records from this project are also published to the Global Biodiversity Information Facility (GBIF, <https://www.gbif.org/>) via the VertNet Integrated Publishing Toolkit (<http://ipt.vertnet.org/>). &lquo;Research grade&rquo; iNaturalist observations are also published to GBIF.
...
We were not able to distinguish species of *Enallagma* (Odonata: Coenagrionidae) because the sequence was one of multiple species sharing identical or nearly identical DNA barcode sequences in BIN BOLD:AAA2218, which includes multiple Nearctic species [@Geigeretal2021].
...
In all except the plankton samples, we generally observed less diversity in the metabarcoding results than we expected. We suspect that this may have been due to subsampling or inadequate homogenization of samples, resulting in some species not being included in DNA extractions. In the future, we may homogenize samples using a blender and cleaning between samples using DIY-DS as described by @Buchneretal2021 before shipping samples out for metabarcoding.

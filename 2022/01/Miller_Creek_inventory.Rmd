...
*by Matthew L. Bowser^[USFWS Kenai National Wildlife Refuge, Soldotna, Alaska, matt_bowser@fws.gov],
 Samuel I. Artaiz^[USDA NRCS/Harney Soil and Water Conservation District, Hines, Oregon],
 ...
 Benjamin Meyer^[Kenai Watershed Forum, Soldotna, Alaska], and
 Dom Watts^[USFWS Kenai National Wildlife Refuge, Soldotna, Alaska] 
 <!--Chelsea Wisotzkey^[Kenai Watershed Forum, Soldotna, Alaska], -->
 <!-- Warren R. Wyrick^[Alaska Department of Fish and Game, Soldotna, Alaska] -->
*

## Abstract

Because benthic macroinvertebrates and zooplankton are susceptible to the pesticide rotenone, surveys of freshwater macroinvertebrates were conducted in the Miller Creek Watershed, Kenai Peninsula, Alaska ahead of a planned rotenone treatment in fall 2021. In 2021 we collected 32 mixed invertebrate samples to be processed by metabarcoding. To analyze the resulting sequence data we compared the analysis delivered by a commercial service with the results of a customized analysis pipeline using Je, MetaWorks, and local reference libraries. In the metagenomic data, the commercial pipeline yielded 2,250 exact sequence variants (ESVs) representing 84 unique identifications; the Je + MetaWorks pipeline yielded 277 ESVs and 175 unique identifications. The results of the two pipelines were similar overall. In addition, we documented 140 occurrences of 22 species based on specimens and photo observations. Overall, our metagenomic methods delivered finer taxonomic resolution than comparable inventories of invertebrates performed ahead of rotenone treatments.

## Introduction

Invasive northern pike (*Esox lucius* Linnaeus, 1758) were discovered in the Miller Creek watershed in 2019 [@KNWR2021] and a rotenone treatment to eradicate pike from the drainage was planned for fall 2021, part of a larger effort to eradicate nothern pike from the Kenai Peninsula [@SANPCC2007].

Benthic macroinvertebrates and zooplankton are susceptible to rotenone [@Finlaysonetal2018], some of them dying at lower rotenone concentrations than fish [@Rach1988]. Planktonic crustaceans decline rapidly after rotenone treatments, generally taking months to years to fully recover [@Kiseretal1963; @Anderson1970]. In other southcentral Alaska lakes, most pre-treatment plankton species were present in lake water with in one to two years of treatment and plankton abundace returned to pre-treatment levels in the third year [@Chlupach1977]. On the Kenai Peninsula, most invertebrate taxa that were present in lakes before present were again present one year after treatment [@Massengill2014; @Massengill2017]. 
...
We generally followed the methods of @Massengill2014 and @Massengill2017 for sampling aquatic invertebrates. Each of the three waterbodies was to be surveyed once in July and once in August. Sampling was to be conducted in the same temporal windows in 2021 and 2022.
...
```{r millermap, fig.cap="Map of the Miller Creek watershed showing sample locations.", out.width="65%"}
...
For identifying specimens we used the following references: @Borroretal1989, @Brooks1957, @Burch1982, @Collet2008, @Durfee2005, @Haneyetal2013, @Hatch1953, @Herrington1962, @Kenner2009, @Mackie2007, @Marx1957, @MerrittCummins1996, @Merrittetal2008, @Reid1987, @Rileyetal2002, @Roughley2000, @Walker1953, @Wallis1933, and @White1983.
...
Sequence data were processed by using MR DNA analysis pipeline (MR DNA, Shallowater, Texas).  In summary, sequences were joined, sequences < 150 bp were removed, and sequences with ambiguous base calls were removed. Sequences were quality filtered using a maximum expected error threshold of 1.0 and dereplicated. The dereplicated or unique sequences were denoised and chimeras were removed. Final zero-radius operational taxonomic units were taxonomically classified using BLASTn against a curated database derived from NCBI (<https://www.ncbi.nlm.nih.gov/>).   
...
nspecimens <- nrow(sr)
nphoto <- nrow(or) - length(dups)
nspecimens <- nspecimens - nphoto
...
The metazoan portion of the metabarcoding dataset provided by MR DNA included a total of 1,050,965 reads representing 2,250 ESVs. After consolidating ESVs having identical identifications, 84 unique identifications were retained. The three most abundant identifications were *Gammarus kischineffensis* with 149,335 reads, *Glossiphonia complanata* with 69,285 reads, and *Enallagma* sp. bioug23416_h08 with 66,795 reads (Table \@ref(tab:comptab)).
...
In the metabarcoding dataset resulting from the Je + MetaWorks pipeline, a total of 589,486 reads representing 277 ESVs were retained after filtering. After consolidating ESVs having identical identifications, there were 175 unique identifications. The three most abundant identifications were *Gammarus lacustris* with 86,576 reads, *Enallagma* sp. BOLD:AAA2218 with 73,898 reads, and *Sphaerium nitidum* with 41,880 (Table \@ref(tab:comptab)). Read abundances of these and all other identifications are represented by circle sizes in Figure \@ref(fig:chordplot).
...
In addition to the metabarcoding samples, we documented `r nspecimens` specimen-based occurrences and `r nphoto` photo observations representing `r nphyl` phyla, `r nclas` classes, `r nord` orders, `r nfam` families, `r ngen` genera, and `r nspp` species (Table \@ref(tab:specimentable)). 
...
A corresponding project record was also made on ServCat [@Bowser2022proj], which includes the command line and R scripts used in the analysis, photographs, and other information. Raw and demultiplexed metabarcoding data are available from ServCat [@Bowser2022raw]. Resulting occurrences have been published to GBIF as a dataset [@Bowseretal2022].
...
spectab <- knitr::kable(sa[,c(7,2,1)], 
  ...
  row.names = FALSE,
  caption = "Identifications of specimens and photo observations by water body. Complete specimen and photo observation data are available from the URLs provided below."
)
...
We documented `r nrec` new distribution records for the Kenai National Wildlife Refuge: `r reclist`. These were added the Refuge's species list in FWSpecies [@NRPC2019] the checklist data have been made available via a GitHub repository (<https://github.com/Kenai-NWR-bio/KenaiNWRspecies>).
...
```
Because of the large size differences between the invertebrates we collected (from > 20 mm snails and dragonfly nymphs down to minute, < 1 mm plankton), which can result in smaller species not being detected, we may sort specimens by size or take smaller tissue samples from large specimens when we resurvey in 2022. Both of these strategies have been used to improve detection of small species in metagenomic samples [e.g., @Jietal2013; @Elbrechtetal2017; @Carewetal2018; @Elbrechtetal2020; @Buchneretal2021].
...
Overall, we believe that the metagenomic approach was superior to similar studies based on morphological specimens [e.g., @Massengill2014, @Massengill2017] because taxonomic resolution was finer in the present work than in the previous studies. In particular, small planktonic species were identified efficiently by metabarcoding. We lack the expertise to have identified most planktonic species based on morphology, so we would have needed to send these to an expert for identification.

We suspect that we missed many species that were originally present in the samples due to subsampling by the sequencing labrartory. In 2022 we plan to address this problem by homogenizing samples before shipping them out for sequencing.
...
Matt Bowser and Rob Massengill planned the project. Sam Artaiz, Matt Bowser, Jake Danner, Kris Dent, Ben Meyer, Dom Watts, Chelsea Wisotzkey, and Warren Wyrick participated in field work. Sam Artaiz and Matt Bowser sorted and identified specimens. Matt Bowser processed the data and led writing of the manuscript.

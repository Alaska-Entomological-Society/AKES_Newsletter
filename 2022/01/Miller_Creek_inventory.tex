...
We assembled a reference library by concatenating final Exact Sequence Variant (\acr{ESV}) sequences from \citet{Bowseretal2020, BowserBowser2020} and the DS-BOWSER project on BOLD\footnote{\url{https://boldsystems.org/index.php/Public_SearchTerms?searchMenu=records&query=DS-BOWSER}}, then dereplicating them with \verb|vsearch --derep_fulllength|. We looked for matches with this reference library using \verb|vsearch --usearch_global|. We also compared our sequences to sequences available in BOLD using the bold package \citep{Chamberlain2021}, version 1.2.0.
